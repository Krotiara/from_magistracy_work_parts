...
{
    public class AgentCable : Agent
    {

        private Simulator.Model.Cable cable;
       ...
       // private AgentSag agentSag;
        ...
        public AgentCable(Simulator.Model.Cable cable, float sagNormativeValue, float currentMaxStress, float currentBreakingStress, AgentSpan agentSpan)
        {
            this.cable = cable;
            Number = cable.Number;
            Weight = 0.364f;
            ...
            //B = -0.1969945156695157f;
            //B = -0.36648580095441596f;
            B = -0.5272306803703704f;
            Connections = new List<Agent>();
            this.agentSpan = agentSpan;
            InitParameters();
            SetStateDiagram();
            ObjectName = string.Format("Cable, phase {0}", cable.Phase);
        ...
        public override string GetParamsDescription()
        {
            ...
            res += string.Format("Tracked parameter (GTCI): {0};" +
                 "\nState: {1};\ndebug Ice param = {2}", Math.Round(TrackedParameter, 1).ToString(), StateDiagram.CurrentState.Name, cable.CalcMode.IceThickness);
            return res;
        }
        ...
        public override List<(string, string)> GetParamsDescriptionForTable()
        {
            ...
            res.AddRange(new List<(string, string)>()
            {
                ("Aluminium break rods out clamps count", BreakRodsOutClamps.Where(x => x.Type == Simulator.RodType.Aluminium).Count().ToString()),
                 ...
                 ("Stresses min delta: Stress", stressesWithMinDelta.Item1.ToString()),
                 ("Stresses min delta: Breaking stress", stressesWithMinDelta.Item2.ToString()),
            }
            );
